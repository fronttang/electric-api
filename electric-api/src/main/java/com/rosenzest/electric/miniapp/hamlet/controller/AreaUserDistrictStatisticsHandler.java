...
public class AreaUserDistrictStatisticsHandler implements IAreaUserStatisticsHandler {
	...
	public List<OwnerUnitDangerStatisticsVo> statistics(AreaUserInfoVo userInfo) {
		...
		if (CollUtil.isNotEmpty(userDangers)) {
			...
			Map<String, List<AreaUserDangerVo>> groupMap = userDangers.stream().collect(Collectors
					.groupingBy((d) -> StrUtil.format("{}/{}", d.getDistrict(), d.getStreet()), Collectors.toList()));
